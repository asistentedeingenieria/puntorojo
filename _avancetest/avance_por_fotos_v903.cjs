...
if (src) {
  ...
  // v904: la función devuelve { actual, inferiores } (cascada hacia abajo probada en
  // avance_cascada_orden_v904.cjs — acá solo la conducta v903 de la etapa ACTUAL)

  // 2 fotos y sin marcar → marca esa etapa, con sello (etapas POSTERIORES intactas)
  const a1 = { photos: { '2': ['u1','u2'] }, stages: [true,true,false,false,false,false], stagesTs: [1,2] };
  ok('2 fotos → marcada con sello', f(a1,2).actual==='marcada' && a1.stages[2]===true && typeof a1.stagesTs[2]==='number' && a1.stagesTs[2]>0);
  ok('sin cascada hacia ARRIBA: las posteriores no se tocan', a1.stages[3]===false && a1.stages[4]===false);

  // 1 foto → pide la segunda, NO marca la actual
  const a2 = { photos: { '0': ['u1'] }, stages: [false,false,false,false,false,false] };
  ok('1 foto → falta1 y no marca', f(a2,0).actual==='falta1' && a2.stages[0]===false);
  ...
  ok('ya marcada → no toca el sello', f(a3,1).actual==='ya' && a3.stagesTs[1]===77);

  // 0 fotos / nulls no cuentan
  const a4 = { photos: { '4': [null,null] }, stages: [false,false,false,false,false,false] };
  ok('nulls no cuentan como fotos', f(a4,4).actual==='sin' && a4.stages[4]===false);

  // apto sin arrays → se construyen
  const a5 = { photos: { '0': ['u1','u2'] } };
  ok('apto virgen: construye stages y marca', f(a5,0).actual==='marcada' && a5.stages[0]===true && a5.stages.length===6);
}
